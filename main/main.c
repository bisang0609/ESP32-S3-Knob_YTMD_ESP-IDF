...
static void lcd_backlight_init(uint16_t duty)
{
...
static void lcd_backlight_set(uint16_t duty)
{
...
void example_backlight_test_task(void *arg)
{
    for(;;)
    {
        lcd_backlight_set(LCD_PWM_MODE_255);
        vTaskDelay(pdMS_TO_TICKS(1000));
        lcd_backlight_set(LCD_PWM_MODE_200);
        vTaskDelay(pdMS_TO_TICKS(1000));
        lcd_backlight_set(LCD_PWM_MODE_150);
        vTaskDelay(pdMS_TO_TICKS(1000));
        lcd_backlight_set(LCD_PWM_MODE_100);
        vTaskDelay(pdMS_TO_TICKS(1000));
        lcd_backlight_set(LCD_PWM_MODE_50);
        vTaskDelay(pdMS_TO_TICKS(1000));
        lcd_backlight_set(LCD_PWM_MODE_0);
        vTaskDelay(pdMS_TO_TICKS(1000));
    }
}
#endif

static void ui_update_from_wifi(void)
{
    wifi_ui_snapshot_t snapshot;

    if (!wifi_sta_ui_consume_update(&snapshot)) {
        return;
    }
