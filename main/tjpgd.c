...
/ Modified: removed LV_USE_SJPG guard for standalone project use
/----------------------------------------------------------------------------*/
...
static JRESULT restart(JDEC* jd, uint16_t rstn)
{
    ...
    if ((marker & 0xFFD8) != 0xFFD0 || (marker & 7) != (rstn & 7)) return JDR_FMT1;
    jd->dbit = 0;
#endif
...
static JRESULT mcu_output(JDEC* jd, int (*outfunc)(JDEC*, void*, JRECT*), unsigned int img_x, unsigned int img_y)
{
            ...
            for (iy = 0; iy < my; iy += w) {
                for (ix = 0; ix < mx; ix += w) {
                    ...
                    for (y = 0; y < w; y++) {
                        for (x = 0; x < w; x++) {
                            r += *pix++;
                            if (JD_FORMAT != 2) { g += *pix++; b += *pix++; }
                        }
                        pix += a;
                    }
                    *op++ = (uint8_t)(r >> s);
                    if (JD_FORMAT != 2) { *op++ = (uint8_t)(g >> s); *op++ = (uint8_t)(b >> s); }
                }
            }
        }
    } else {
...
JRESULT jd_prepare(JDEC* jd, size_t (*infunc)(JDEC*, uint8_t*, size_t), void* pool, size_t sz_pool, void* dev)
{
    ...
    jd->pool = pool; jd->sz_pool = sz_pool;
    jd->infunc = infunc; jd->device = dev;
